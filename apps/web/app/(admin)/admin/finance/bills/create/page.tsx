        ...
        <div className="p-6 max-w-4xl mx-auto space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg border shadow-sm">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><label className="block text-sm font-medium mb-1">Vendor Invoice #</label><input className="w-full border rounded p-2" value={vendorInvoiceNumber} onChange={e => setVendorInvoiceNumber(e.target.value)} /></div>
                    <div><label className="block text-sm font-medium mb-1">Bill Date</label><input type="date" className="w-full border rounded p-2" value={date} onChange={e => setDate(e.target.value)} required /></div>
                    <div><label className="block text-sm font-medium mb-1">Due Date</label><input type="date" className="w-full border rounded p-2" value={dueDate} onChange={e => setDueDate(e.target.value)} required /></div>
                </div>
