...
export default function EditSalesOrderPage() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setLoading(true);
        try {
            const res = await fetch(`/api/finance/sales-orders/${params!.id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ customerName, date, items, customerEmail, customerPhone, customerAddress })
            });
            if (res.ok) router.push(`/admin/finance/sales-orders/${params!.id}`);
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
    ...
    if (fetching) return <div className="p-12 flex justify-center"><Loader2 className="w-8 h-8 animate-spin text-indigo-600" /></div>;

    return (
        <div className="p-6 max-w-4xl mx-auto space-y-6">
            <BackButton fallbackUrl={`/admin/finance/sales-orders/${params!.id}`} />
