...
export async function GET(req: NextRequest) {
  try {
    ...
    const [
      ...
      prisma.$queryRaw`SELECT COUNT(*)::int as count FROM registrations r JOIN events e ON r.event_id = e.id WHERE ${whereTenantEvents} AND r.status IN ('APPROVED', 'PENDING')`,
      prisma.$queryRaw`
        SELECT COALESCE(SUM(r.price_inr), 0)::int as revenue 
        FROM registrations r 
        JOIN events e ON r.event_id = e.id 
        WHERE ${whereTenantEvents} AND r.status IN ('APPROVED', 'PENDING')
      `,
      prisma.$queryRaw`
        SELECT 
          e.id::text,
          ...
          (SELECT COUNT(*)::int FROM registrations r WHERE r.event_id = e.id AND r.status IN ('APPROVED', 'PENDING')) as registrations,
          (SELECT COUNT(*)::int FROM rsvp_responses rr WHERE rr.event_id = e.id) as rsvps,
          COALESCE(e.price_inr, 0)::int as price,
          (SELECT COALESCE(SUM(r.price_inr), 0)::int FROM registrations r WHERE r.event_id = e.id AND r.status IN ('APPROVED', 'PENDING')) as revenue
        FROM events e
        LEFT JOIN tenants t ON e.tenant_id = t.id
        WHERE ${whereTenantEvents}
        ORDER BY (SELECT COUNT(*)::int FROM registrations r WHERE r.event_id = e.id AND r.status IN ('APPROVED', 'PENDING')) DESC
        LIMIT 10
      `,
      prisma.$queryRaw`
        SELECT 
