...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const evKey = `event:${params.id}`

    const [vendorsKV, budgetsKV] = await Promise.all([
      prisma.keyValue.findFirst({ where: { namespace: VNS, key: evKey }, select: { value: true } }),
      prisma.keyValue.findFirst({ where: { namespace: BNS, key: evKey }, select: { value: true } }),
    ])
    ...
    for (const cat of categories) {
      ...
      const r = b - s
      rows.push([toCsvValue(cat), toCsvValue(b), toCsvValue(s), toCsvValue(r)].join(','))
    }
    ...
    return new NextResponse(csv, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="budgets_${params.id}.csv"`,
