  ...
  try {
    ...
    const result = await prisma.$queryRaw`
      ...
      SET
        ...
        contact_data = ${JSON.stringify(raw.contactData || {}) || '{}'},
        payment_data = ${JSON.stringify(raw.paymentData || {}) || '{}'},
        branding_online = ${JSON.stringify(raw.brandingOnline || {}) || '{}'},
        branding_offline = ${JSON.stringify(raw.brandingOffline || {}) || '{}'},
        event_presence = ${JSON.stringify(raw.eventPresence || {}) || '{}'},
        giveaway_data = ${JSON.stringify(raw.giveawayData || {}) || '{}'},
        legal_data = ${JSON.stringify(raw.legalData || {}) || '{}'},
        timeline_data = ${JSON.stringify(raw.timelineData || {}) || '{}'},
        post_event_data = ${JSON.stringify(raw.postEventData || {}) || '{}'},
        updated_at = NOW()
