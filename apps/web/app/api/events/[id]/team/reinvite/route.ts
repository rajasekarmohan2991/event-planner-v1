...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const res = await fetch(`${API_BASE}/events/${params.id}/team/reinvite${qp}`, {
      method: 'POST',
    ...
    if (!res.ok) return NextResponse.json(payload || { message: 'Reinvite failed' }, { status: res.status })
    return NextResponse.json(payload)
  } catch (e: any) {
    return NextResponse.json({ message: e?.message || 'Reinvite failed' }, { status: 500 })
  }
}
