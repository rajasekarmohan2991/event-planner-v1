...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    const res = await fetch(`${API_BASE}/events/${params.id}/payment-settings`, {
    ...
    if (!res.ok) return NextResponse.json(payload || { message: 'Fetch failed' }, { status: res.status })
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const isJson = (res.headers.get('content-type') || '').includes('application/json')
    const payload = isJson && text ? JSON.parse(text) : (text ? { message: text } : {})
    if (!res.ok) return NextResponse.json(payload || { message: 'Save failed' }, { status: res.status })
    return NextResponse.json(payload)
  } catch (e: any) {
    return NextResponse.json({ message: e?.message || 'Save failed' }, { status: 500 })
  }
}
