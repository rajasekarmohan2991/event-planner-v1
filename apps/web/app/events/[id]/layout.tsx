...
export default function EventWorkspaceLayout({ children, params }: { children: React.ReactNode; params: { id: string } }) {
  ...
  // Check event existence (handles permanently deleted events)
  useEffect(() => {
    let aborted = false
    ;(async () => {
      try {
        ...
        if (aborted) return
        if (res.status === 404) { setExists('no'); return }
        if (!res.ok) { setExists('no'); return }
        // Try to read the event details for title display
        try {
          const data = await res.json()
          const title = data?.name || data?.title || ''
          if (!aborted) setEventTitle(title)
        } catch {}
        setExists('yes')
      } catch {
        if (!aborted) setExists('no')
      }
    })()
    return () => { aborted = true }
  }, [params.id])
            ...
            if (href === regBase) {
              return (
                <div key={href} className="space-y-1">
                  <div
                    className={`w-full group flex items-center justify-between gap-2 rounded-md px-3 py-2 text-sm transition-colors ${
                      isOnRegistrations || regOpen
