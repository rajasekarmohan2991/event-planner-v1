...
export function SeatSelector({ eventId, ticketClassId, onSeatSelect, maxSeats = 4 }: SeatSelectorProps) {
  ...
  const fetchSeats = async (silent = false) => {
    try {
      ...
      // Auto-generation removed: Seats must be generated by Admin to prevent unauthorized POST requests
      if (!triedGenerateRef.current && newSeats.length === 0 && data.floorPlan) {
        // Just mark as tried so we don't retry, but DO NOT call generate endpoint
